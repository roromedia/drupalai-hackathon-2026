...
use Drupal\content_preparation_wizard\Exception\CanvasCreationException;
use Drupal\content_preparation_wizard\Model\ContentPlan;
use Drupal\content_preparation_wizard\Model\PlanSection;
use Drupal\content_preparation_wizard\Service\CanvasCreatorInterface;
...
class Step3CreateForm extends FormBase {
   ...
   *
   * @param \Drupal\content_preparation_wizard\Service\WizardSessionManagerInterface $sessionManager
   *   The wizard session manager.
   * @param \Drupal\content_preparation_wizard\Service\CanvasCreatorInterface $canvasCreator
  ...
  public static function create(ContainerInterface $container): static {
    return new static(
      ...
      $container->get('content_preparation_wizard.canvas_creator'),
  ...
  public function getFormId(): string {
    return 'content_preparation_wizard_step3';
  }
  ...
  protected function buildActionsSection(ContentPlan $plan): array {
    ...
    $section['back'] = [
      ...
      '#url' => Url::fromRoute('content_preparation_wizard.wizard.step', ['step' => 'plan']),
      '#attributes' => [
        'class' => ['button'],
      ],
    ];
    ...
    $section['cancel'] = [
      '#type' => 'link',
      '#title' => $this->t('Cancel'),
      '#url' => Url::fromRoute('content_preparation_wizard.wizard'),
      '#attributes' => [
        'class' => ['button'],
      ],
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    if ($session === NULL) {
      ...
      $form_state->setRedirect('content_preparation_wizard.wizard');
      return;
    }
    ...
    if ($plan === NULL) {
      ...
      $form_state->setRedirect('content_preparation_wizard.wizard.step', ['step' => 'plan']);
      return;
    }
    ...
    try {
      ...
      $this->getLogger('content_preparation_wizard')->error('Canvas page creation failed: @message', [
      ...
      try {
        ...
        $this->getLogger('content_preparation_wizard')->warning('Failed to update plan status: @message', [
      ...
      $this->getLogger('content_preparation_wizard')->error('Unexpected error during Canvas page creation: @message', [
        '@message' => $e->getMessage(),
        'exception' => $e,
      ]);
