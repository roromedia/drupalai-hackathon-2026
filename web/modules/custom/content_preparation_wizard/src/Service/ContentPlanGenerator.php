...
use Drupal\content_preparation_wizard\Model\RefinementEntry;
...
class ContentPlanGenerator implements ContentPlanGeneratorInterface {
  ...
  public function __construct(
    ...
    $this->logger = $this->loggerFactory->get('content_preparation_wizard');
  }
   ...
   *
   * @param array<\Drupal\content_preparation_wizard\Model\ProcessedDocument> $documents
   ...
   *
   * @param array<\Drupal\content_preparation_wizard\Model\AIContext> $contexts
   *   The AI contexts.
   *
   * @return string
...
PROMPT;
  ...
  protected function executeAiCallWithRetries(mixed $provider, string $modelId, string $systemPrompt, string $userMessage): array {
    ...
    for ($attempt = 1; $attempt <= self::MAX_RETRIES; $attempt++) {
      try {
        ...
        // Execute chat.
        $response = $provider->chat($chatInput, $modelId, ['content_preparation_wizard']);
   ...
   *
   * @return \Drupal\content_preparation_wizard\Model\PlanSection
   *   The parsed section.
   */
   ...
   * @param array<string, mixed> $data
   *   The parsed JSON data.
   * @param \Drupal\content_preparation_wizard\Model\ContentPlan $originalPlan
