...
```php
// Store wizard data
$this->tempStoreFactory->get('content_preparation_wizard')
  ->set('wizard_data', [
...
interface DocumentProcessorInterface extends PluginInspectionInterface {
   ...
   *   The file to process.
   *
   * @return \Drupal\content_preparation_wizard\ProcessedDocument
   *   The processed document with extracted content.
   *
   * @throws \Drupal\content_preparation_wizard\Exception\ProcessingException
   */
...
public function generatePlan(
  ...
  $template = $this->configFactory->get('content_preparation_wizard.settings')
...
class ContentPreparationSettingsForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array {
    $config = $this->config('content_preparation_wizard.settings');
...
The architecture supports a `content_preparation_wizard_scraper` submodule that:
